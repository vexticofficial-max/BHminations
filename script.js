...
function openVideoModal(video) {
    ...
    currentVideo = {
        ...video,
        authorUID: video.authorUID || video.authorId || '',
        author: video.author || video.authorName || 'Bilinmeyen Kanal'
    };
    ...
    if (currentVideo.videoId) {
        ...
        iframe.src = `https://www.youtube.com/embed/${currentVideo.videoId}?autoplay=1`;
        playerContainer.appendChild(iframe);
    ...
    if (detailTitle) detailTitle.textContent = currentVideo.title || 'Başlıksız';
    if (detailViews) detailViews.textContent = `${currentVideo.views || 0} izlenme`;
    if (detailDate) detailDate.textContent = formatTime(currentVideo.uploadedAt);
    if (detailAuthor) detailAuthor.textContent = currentVideo.author;
    if (detailAvatar) detailAvatar.src = currentVideo.authorAvatar || 'https://via.placeholder.com/40?text=A';
    if (detailDescription) detailDescription.textContent = currentVideo.description || 'Açıklama bulunamadı';
    if (likeCount) likeCount.textContent = currentVideo.likes || 0;
    if (dislikeCount) dislikeCount.textContent = currentVideo.dislikes || 0;

    db.collection('videos').doc(currentVideo.id).update({
        views: firebase.firestore.FieldValue.increment(1)
    }).catch(() => {});
...
async function updateSubscribeButton() {
    ...
    const snapshot = await subscriptionRef.get();
    btn.textContent = snapshot.empty ? 'Abone Ol' : 'Aboneliği Kaldır';
}
...
async function likeVideo() {
    ...
    try {
        const videoRef = db.collection('videos').doc(currentVideo.id);
        const likeRef = db.collection('videoLikes')
            .where('videoId', '==', currentVideo.id)
            .where('userId', '==', currentUser.uid);
        const dislikeRef = db.collection('videoDislikes')
            .where('videoId', '==', currentVideo.id)
            .where('userId', '==', currentUser.uid);

        const [likeSnap, dislikeSnap] = await Promise.all([likeRef.get(), dislikeRef.get()]);
        ...
        if (likeSnap.empty) {
            ...
            document.getElementById('like-count').textContent = currentVideo.likes;

            if (!dislikeSnap.empty) {
                dislikeSnap.forEach(doc => doc.ref.delete());
                await videoRef.update({ dislikes: firebase.firestore.FieldValue.increment(-1) });
                currentVideo.dislikes = Math.max(0, (currentVideo.dislikes || 0) - 1);
                document.getElementById('dislike-count').textContent = currentVideo.dislikes;
            }
        } else {
            likeSnap.forEach(doc => doc.ref.delete());
            await videoRef.update({ likes: firebase.firestore.FieldValue.increment(-1) });
...
async function dislikeVideo() {
    ...
    try {
        ...
        const [dislikeSnap, likeSnap] = await Promise.all([dislikeRef.get(), likeRef.get()]);
...
function shareVideo(videoId) {
    const shareId = videoId || currentVideo?.id;
    if (!shareId) return;
    const base = window.location.href.split('?')[0];
    const url = `${base}?v=${encodeURIComponent(shareId)}`;
    if (navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(url).then(() => {
            alert('Video linki kopyalandı!');
        }).catch(() => alert('Link kopyalanamadı: ' + url));
    } else {
        alert('Video linki: ' + url);
    }
}
...
async function loadComments(videoId) {
    const commentsList = document.getElementById('comments-list');
    const commentInputArea = document.getElementById('comment-input-area');
    if (!commentsList || !commentInputArea) return;
    ...
    if (currentUser) {
        ...
        commentInputArea.innerHTML = '<p style="color:var(--secondary-text);">Yorum yapmak için giriş yapın.</p>';
    }
    ...
    roots.forEach(comment => {
        ...
        commentEl.className = 'comment-item';
        commentEl.innerHTML = `
            <img src="${comment.userAvatar || 'https://via.placeholder.com/36?text=U'}" class="comment-avatar">
            <div class="comment-content">
                <div class="comment-author">${escapeHtml(comment.userName || 'Kullanıcı')}</div>
                <div class="comment-text">${escapeHtml(comment.text || '')}</div>
                <div class="comment-actions">
                    <span onclick="likeComment('${comment.id}')">👍 ${comment.likes || 0}</span>
                    <span onclick="replyComment('${comment.id}')">Yanıtla</span>
                </div>
                <div id="reply-box-${comment.id}" style="display:none; margin-top:8px;">
                    <textarea id="reply-input-${comment.id}" placeholder="Yanıt yaz..." style="width:100%; padding:6px; border:1px solid var(--border-color); border-radius:4px; min-height:52px;"></textarea>
                    <div style="margin-top:6px; display:flex; gap:8px;">
                        <button class="btn-primary" style="padding:5px 12px; font-size:12px;" onclick="postReply('${comment.id}')">Gönder</button>
                        <button style="padding:5px 12px; border:1px solid var(--border-color); background:none; cursor:pointer; border-radius:4px;" onclick="replyComment('${comment.id}')">İptal</button>
                    </div>
                </div>
                <div id="replies-${comment.id}" style="margin-top:8px; padding-left:12px; border-left:2px solid var(--border-color);"></div>
            </div>
        `;
        commentsList.appendChild(commentEl);

        const repliesEl = commentEl.querySelector(`#replies-${comment.id}`);
        (repliesMap[comment.id] || []).forEach(reply => {
            const replyItem = document.createElement('div');
            replyItem.style.marginBottom = '8px';
            replyItem.innerHTML = `
                <div style="font-size:12px; color:var(--secondary-text);">${escapeHtml(reply.userName || 'Kullanıcı')}</div>
                <div style="font-size:13px;">${escapeHtml(reply.text || '')}</div>
            `;
...
async function postComment(videoId) {
    ...
    if (vidDoc.exists) {
        const vid = vidDoc.data();
        if (vid.authorUID && vid.authorUID !== currentUser.uid) {
            sendNotification(vid.authorUID, `${currentUser.displayName} videonuza yorum yaptı`, videoId);
        }
    }
}

async function likeComment(commentId) {
    if (!currentUser) return alert('Yorum beğenmek için giriş yapın');
    const commentRef = db.collection('comments').doc(commentId);
    const voteRef = db.collection('commentLikes')
        .where('commentId', '==', commentId)
        .where('userId', '==', currentUser.uid);
    const voteSnap = await voteRef.get();

    if (voteSnap.empty) {
        await commentRef.update({ likes: firebase.firestore.FieldValue.increment(1) });
        ...
        voteSnap.forEach(doc => doc.ref.delete());
        await commentRef.update({ likes: firebase.firestore.FieldValue.increment(-1) });
    }

    if (currentVideo?.id) loadComments(currentVideo.id);
}
...
async function postReply(commentId) {
    if (!currentUser || !currentVideo) return alert('Yanıt yazmak için giriş yapın');
    const input = document.getElementById(`reply-input-${commentId}`);
...
async function toggleDetailSubscribe() {
    if (!currentUser || !currentVideo?.authorUID) return alert('Lütfen giriş yapın');
    ...
    if (snapshot.empty) {
        await db.collection('subscriptions').add({
            subscriberId: currentUser.uid,
            channelId: currentVideo.authorUID,
            channelName: currentVideo.author || 'Kanal',
            channelAvatar: currentVideo.authorAvatar || '',
            timestamp: firebase.firestore.FieldValue.serverTimestamp()
        });
    } else {
        snapshot.forEach(doc => doc.ref.delete());
    ...
    db.collection('subscriptions')
        ...
        .onSnapshot(async snapshot => {
            ...
            for (const doc of snapshot.docs) {
                const sub = doc.data();
                let channelName = sub.channelName || 'Kanal';
                let channelAvatar = sub.channelAvatar || '';

                if (sub.channelId && (!sub.channelName || !sub.channelAvatar)) {
                    const channelDoc = await db.collection('users').doc(sub.channelId).get();
                    if (channelDoc.exists) {
                        const channel = channelDoc.data();
                        channelName = channel.displayName || channelName;
                        channelAvatar = channel.avatar || channelAvatar;
                    }
                }
                ...
                subItem.className = 'sidebar-item';
                subItem.innerHTML = `
                    <img src="${channelAvatar || 'https://via.placeholder.com/24?text=C'}" style="width:24px;height:24px;border-radius:50%;object-fit:cover;">
                    <span>${escapeHtml(channelName)}</span>
                `;
                subItem.onclick = (e) => {
                    e.preventDefault();
                    if (sub.channelId) openChannel(sub.channelId);
                };
                subList.appendChild(subItem);
            }
        });
